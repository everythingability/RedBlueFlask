...
function yellowsetup() {
	...
	for (let i = 0; i < nGestures; i++) {
	  gestureArray[i] = new Gesture(buffer.width, buffer.height);
	}
    ...
    if (gesture.exists) {
        if (gesture.nPolys > 0) {
            ...
            for (let i = 0; i < gnp; i++) {
                ...
                buffer.vertex(xpts[0], ypts[0]);
                buffer.vertex(xpts[1], ypts[1]);
                buffer.vertex(xpts[2], ypts[2]);
                buffer.vertex(xpts[3], ypts[3]);

                if ((cr = crosses[i]) > 0) {
                    if ((cr & 3)>0) {
                        buffer.vertex(xpts[0]+w, ypts[0]);
                        buffer.vertex(xpts[1]+w, ypts[1]);
                        buffer.vertex(xpts[2]+w, ypts[2]);
                        buffer.vertex(xpts[3]+w, ypts[3]);
                    ...
                    if ((cr & 12)>0) {
                        buffer.vertex(xpts[0], ypts[0]+h);
                        buffer.vertex(xpts[1], ypts[1]+h);
                        buffer.vertex(xpts[2], ypts[2]+h);
                        buffer.vertex(xpts[3], ypts[3]+h);
